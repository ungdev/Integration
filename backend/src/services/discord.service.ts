...
import { discord_client_id, discord_client_secret, discord_redirect_uri } from '../shared/secrets/secrets';
...
export const syncDiscordUserId = async (code: string, userId: number) => {
    ...
    await db.update(userSchema).set({ discord_id: userResponse.data.id }).where(eq(userSchema.id, userId));
