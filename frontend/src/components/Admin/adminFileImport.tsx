...
export const AdminFileImport = (
    ...
    const submitDraftUpload = useCallback(async (itemOverride?: string): Promise<string | null> => {
        if (!draft || !draftFile) {
            return null;
        }
    ...
    return (
        <Card className="w-full max-w-3xl mx-auto gap-3">
            <CardContent>
                        ...
                        {(existingFile?.exists || !!draftPreviewURL) && (
                            <>
                                {(draftPreviewURL || fileURL) && (
                                    <a href={draftPreviewURL ?? fileURL ?? "#"} target="_blank" rel="noopener noreferrer">
                                ...
                                <button onClick={handleDelete} className="cursor-pointer p-2 transition-all duration-200 hover:bg-gray-100 rounded">
